...
const PhotoUpload = ({ eventId, userId }: { eventId: string; userId: string }) => {
  ...
  return (
    <div className="p-4 max-w-md mx-auto">
      ...
      {status === "success" && <p className="text-green-500 mt-2">Photo uploaded for review!</p>}
      {status === "error" && <p className="text-red-500 mt-2">Upload failed.</p>}
    </div>
  );
};
